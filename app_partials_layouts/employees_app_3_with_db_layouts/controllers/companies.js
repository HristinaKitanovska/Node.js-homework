...
const getAll = async (req, res) => {
  ...
  res.render('companies/index', { companies, title: 'Company homepage' });
};

const getCreate = (req, res) => {
  res.render('companies/create', { title: 'Company create' });
};

const getEdit = async (req, res) => {
  const company = await Company.findById(req.params.id);

  res.render('companies/edit', { company, title: 'Company edit' });
}

const getView = async (req, res) => {
  const company = await Company.findById(req.params.id).populate('employees');

  res.render('companies/view', { company, title: 'Company view' });
} 
